...
from muicebot.plugin.hook import on_before_completion, on_after_completion, on_before_pretreatment
from muicebot.llm import ModelRequest, ModelCompletions
from muicebot.models import Resource, Message
...
class Paramparameters(BaseModel):
    ...
    requirements: Optional[list[str]] = Field(description="(可选)第三方依赖列表，默认已安装numpy pandas matplotlib", default=None)
...
async def run_python_code(event: Event, code: str, file_ids: Optional[list[str]] = None, requirements: Optional[list[str]] = None) -> str:
